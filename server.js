...
async function handleAgendaApi(req, res) {
  ...
  if (route.startsWith("/admin/availability/") && req.method === "PUT") {
    const id = route.split("/").pop();
    const payload = await parseBody(req);
    const item = data.availability.find(entry => entry.id === id);
    const start = new Date(payload.start);
    const end = new Date(payload.end);
    if (!item || !payload.start || !payload.end || Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) {
      sendJson(res, 400, { error: "Beschikbaarheid kon niet worden aangepast" });
      return;
    }
    item.start = start.toISOString();
    item.end = end.toISOString();
    item.note = String(payload.note || "Beschikbaar").trim();
    item.updatedById = account.id;
    item.updatedByName = account.username;
    item.updatedByEmail = account.email;
    item.updatedAt = new Date().toISOString();
    addAudit(data, account, "Beschikbaarheid aangepast", `${item.note}: ${item.start} tot ${item.end}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }
  ...
  if (route.startsWith("/admin/unavailable/") && req.method === "PUT") {
    const id = route.split("/").pop();
    const payload = await parseBody(req);
    const item = data.unavailable.find(entry => entry.id === id);
    const start = new Date(payload.start);
    const end = new Date(payload.end);
    if (!item || !payload.start || !payload.end || Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) {
      sendJson(res, 400, { error: "Onbeschikbare tijd kon niet worden aangepast" });
      return;
    }
    item.start = start.toISOString();
    item.end = end.toISOString();
    item.reason = String(payload.reason || "Onbeschikbaar").trim();
    item.updatedById = account.id;
    item.updatedByName = account.username;
    item.updatedByEmail = account.email;
    item.updatedAt = new Date().toISOString();
    addAudit(data, account, "Onbeschikbare tijd aangepast", `${item.reason}: ${item.start} tot ${item.end}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route.startsWith("/admin/bookings/") && req.method === "PUT") {
    const id = route.split("/").pop();
    const payload = await parseBody(req);
    const booking = data.bookings.find(item => item.id === id);
    const start = new Date(payload.start);
    const duration = Number(payload.duration);
    if (!booking || !payload.name || !payload.email || !payload.meetingType || Number.isNaN(start.getTime()) || !data.settings.durations.includes(duration)) {
      sendJson(res, 400, { error: "Afspraak kon niet worden aangepast" });
      return;
    }
    booking.name = String(payload.name).trim();
    booking.organization = String(payload.organization || "").trim();
    booking.email = String(payload.email).trim();
    booking.phone = String(payload.phone || "").trim();
    booking.meetingType = String(payload.meetingType).trim();
    booking.note = String(payload.note || "").trim();
    booking.start = start.toISOString();
    booking.end = new Date(start.getTime() + duration * 60000).toISOString();
    booking.duration = duration;
    booking.status = String(payload.status || "Wacht op bevestigingsmail").trim();
    booking.updatedById = account.id;
    booking.updatedByName = account.username;
    booking.updatedByEmail = account.email;
    booking.updatedAt = new Date().toISOString();
    addAudit(data, account, "Afspraak aangepast", `${booking.name}: ${booking.start}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route.startsWith("/admin/bookings/") && req.method === "DELETE") {
    const id = route.split("/").pop();
    const booking = data.bookings.find(item => item.id === id);
    data.bookings = data.bookings.filter(item => item.id !== id);
    addAudit(data, account, "Afspraak verwijderd", booking ? `${booking.name}: ${booking.start}` : id);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }
  ...
  if (route.startsWith("/admin/accounts/") && req.method === "PUT") {
    const id = route.split("/").pop();
    const payload = await parseBody(req);
    const accountToEdit = data.accounts.find(item => item.id === id);
    if (!accountToEdit || !payload.username || !payload.email) {
      sendJson(res, 400, { error: "Account kon niet worden aangepast" });
      return;
    }
    if (data.accounts.some(item => item.id !== id && (item.username === payload.username || item.email === payload.email))) {
      sendJson(res, 409, { error: "Gebruikersnaam of e-mail bestaat al" });
      return;
    }
    accountToEdit.username = String(payload.username).trim();
    accountToEdit.email = String(payload.email).trim();
    if (String(payload.password || "").trim()) {
      accountToEdit.passwordHash = hashPassword(String(payload.password));
    }
    accountToEdit.updatedAt = new Date().toISOString();
    addAudit(data, account, "Medewerkersaccount aangepast", `${accountToEdit.username} · ${accountToEdit.email}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route.startsWith("/admin/accounts/") && req.method === "DELETE") {
    const id = route.split("/").pop();
    if (id === account.id) {
      sendJson(res, 400, { error: "U kunt uw eigen account niet verwijderen terwijl u bent ingelogd" });
      return;
    }
    const accountToDelete = data.accounts.find(item => item.id === id);
    data.accounts = data.accounts.filter(item => item.id !== id);
    addAudit(data, account, "Medewerkersaccount verwijderd", accountToDelete ? `${accountToDelete.username} · ${accountToDelete.email}` : id);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route === "/admin/followups" && req.method === "POST") {
    const payload = await parseBody(req);
    if (!payload.name || !payload.subject || !payload.body) {
      sendJson(res, 400, { error: "Naam, onderwerp en bericht zijn verplicht" });
      return;
    }
    const template = {
      id: uid("followup-"),
    ...
    data.followUpTemplates.unshift(template);
    addAudit(data, account, "Follow-up template aangemaakt", `${template.name} · ${template.subject}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route.startsWith("/admin/followups/") && req.method === "DELETE") {
    const id = route.split("/").pop();
    const template = data.followUpTemplates.find(item => item.id === id);
    data.followUpTemplates = data.followUpTemplates.filter(item => item.id !== id);
    addAudit(data, account, "Follow-up template verwijderd", template ? `${template.name} · ${template.subject}` : id);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }

  if (route.startsWith("/admin/followups/") && req.method === "PUT") {
    const id = route.split("/").pop();
    const payload = await parseBody(req);
    const template = data.followUpTemplates.find(item => item.id === id);
    if (!template || !payload.name || !payload.subject || !payload.body) {
      sendJson(res, 400, { error: "Template kon niet worden aangepast" });
      return;
    }
    template.name = String(payload.name).trim();
    template.subject = String(payload.subject).trim();
    template.body = String(payload.body).trim();
    template.updatedById = account.id;
    template.updatedByName = account.username;
    template.updatedByEmail = account.email;
    template.updatedAt = new Date().toISOString();
    addAudit(data, account, "Follow-up template aangepast", `${template.name} · ${template.subject}`);
    saveAgendaData(data);
    sendJson(res, 200, { ok: true });
    return;
  }
