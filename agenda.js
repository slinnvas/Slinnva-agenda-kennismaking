...
if (hasAdminPage) {
  ...
  followupForm.addEventListener("submit", async event => {
    event.preventDefault();
    const response = await adminFetch("/api/agenda/admin/followups", {
      method: "POST",
      body: JSON.stringify(Object.fromEntries(new FormData(followupForm).entries()))
    });
    if (response.ok) {
      followupForm.reset();
      await loadAdmin();
      setStatus(adminStatus, "Follow-up e-mailtemplate is opgeslagen.", "success");
    } else {
      const result = await response.json().catch(() => ({}));
