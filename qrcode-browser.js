/* QRCode for JavaScript, copyright (c) 2009 Kazuhiko Arase, MIT License.
   Browser bundle prepared for Slinnva Workshopplatform. */
(function(global){
  var modules={}, cache={};
...
function req(id){if(cache[id])return cache[id].exports;if(!modules[id])throw new Error('QR module not found: '+id);var module=cache[id]={exports:{}};modules[id](module,module.exports,req);return module.exports;}
var QRCode=req('./index'), Level=req('./QRErrorCorrectLevel');
function svg(text){var qr=new QRCode(-1,Level.L);qr.addData(String(text));qr.make();var count=qr.getModuleCount(),quiet=4,size=count+quiet*2,path='';for(var row=0;row<count;row++){for(var col=0;col<count;col++){if(qr.isDark(row,col))path+='M'+(col+quiet)+' '+(row+quiet)+'h1v1h-1z';}}return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+size+' '+size+'" role="img" aria-label="QR-code"><rect width="100%" height="100%" fill="#fff"/><path d="'+path+'" fill="#111" shape-rendering="crispEdges"/></svg>';}
global.SlinnvaQr={svg:svg};
})(window);
